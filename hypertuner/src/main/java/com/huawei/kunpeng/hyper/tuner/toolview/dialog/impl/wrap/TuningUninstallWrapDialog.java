...
import com.huawei.kunpeng.hyper.tuner.action.uninstall.UninstallAction;
...
public class TuningUninstallWrapDialog extends UninstallWrapDialog {
    ...
    protected boolean nextVerify() {
        ...
        if (mainPanel.getAction() instanceof UninstallAction && mainPanel instanceof UninstallPanel) {
            UninstallPanel uninstallPanel = (UninstallPanel) mainPanel;
            UninstallAction uninstallAction = (UninstallAction) mainPanel.getAction();
            uninstallAction.onNextAction(uninstallPanel.getParams(), this);
        }
        // 返回false，不关闭弹框
        return false;
    }
