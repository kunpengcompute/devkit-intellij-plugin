...
public class RunJavaPerfLogDialog extends IdeaDialog {
     ...
     */
    public RunJavaPerfLogDialog(String title, String dialogName, IDEBasePanel panel, boolean resizable) {
        this.title = StringUtil.stringIsEmpty(title) ? RUN_LOG_TITLE : title;
        this.dialogName = StringUtil.stringIsEmpty(dialogName) ? "RunJavaPerfLogDialog" : dialogName;
        mainPanel = panel;
        if (panel instanceof RunJavaPerfLogPanel) {
            runLogPanel = (RunJavaPerfLogPanel) panel;
        }
     ...
     * @param panel 需要展示的面板之一
     */
    public RunJavaPerfLogDialog(String title, IDEBasePanel panel) {
