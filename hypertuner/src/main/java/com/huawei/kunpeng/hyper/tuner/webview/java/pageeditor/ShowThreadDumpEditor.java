...
import com.huawei.kunpeng.hyper.tuner.webview.java.pagewebview.ShowThreadDumpWebView;
 ...
 * @since 2021-08-12
 */
public class ShowThreadDumpEditor extends TuningWebFileEditor {
    private final ShowThreadDumpWebView showThreadDumpWebView;

    public ShowThreadDumpEditor(VirtualFile file) {
        currentFile = file;
        this.showThreadDumpWebView = new ShowThreadDumpWebView();
    }
    ...
    public static void openPage(String pageName) {
        String fileName = new StringBuilder(TuningIDEConstant.TUNING_KPHT)
                ...
                .append(PageType.SHOW_THREAD_DUMP.value())
                .append(SysperfContent.PATH_SEPARATOR)
                .append(pageName)
                .append(".")
    ...
    public JavaWebView getWebView() {
        return showThreadDumpWebView;
    }

    @Override
    public JComponent getComponent() {
        return showThreadDumpWebView.getContent();
    }

    @Override
    public void dispose() {
        super.dispose();
        showThreadDumpWebView.dispose();
    }
}