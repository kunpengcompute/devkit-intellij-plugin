...
public class DiskUtil {
    ...
    public static int queryDiskState() {
        String token = null;
        Object tokenObj =
                PortingIDEContext.getValueFromGlobalContext(PortingIDEConstant.TOOL_NAME_PORTING,
                        BaseCacheVal.TOKEN.vaLue());
        if (tokenObj instanceof String) {
            token = (String) tokenObj;
        }
        RequestDataBean data =
                new RequestDataBean(PortingIDEConstant.TOOL_NAME_PORTING, "/space/", HttpMethod.GET.vaLue(), token);
    ...
    private static int sendNotifyMessage(boolean isWorkNotify,
        ...
        if (isWorkNotify) {
            if (workRemain < THRESHOLD_VALUE) {
                noticeMessage = I18NServer.toLocale("plugins_porting_message_workWarn");
                ...
                noticeMessage = I18NServer.toLocale("plugins_porting_message_workInfo");
                Logger.info("Insufficient workspace of the Porting Advisor. Please delete some reports.");
                res = DISK_STATUS_WORK_YELLOW_ALARM;
            }
        } else {
            if (workRemain < THRESHOLD_VALUE) {
                noticeMessage = I18NServer.toLocale("plugins_porting_message_diskWarn");
                notificationType = NotificationType.ERROR;
                ...
                noticeMessage = I18NServer.toLocale("plugins_porting_message_diskInfo");
        ...
        IDENotificationUtil.notificationCommon(new NotificationBean(
                I18NServer.toLocale("plugins_porting_message_diskNotice"), noticeMessage, notificationType));
        return res;
    }
    ...
    public static void sendDiskAlertMessage() {
        IDENotificationUtil.notificationCommon(new NotificationBean(
                I18NServer.toLocale("plugins_porting_message_diskNotice"),
                I18NServer.toLocale("plugins_porting_message_workDiskError"), NotificationType.ERROR));
