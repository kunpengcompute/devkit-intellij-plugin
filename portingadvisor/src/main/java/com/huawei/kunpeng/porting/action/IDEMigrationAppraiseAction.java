...
/**
 * IDEAnalysisAction: 左侧树软件迁移评估
 *
 * @since 2021-1-05
 */
public class IDEMigrationAppraiseAction extends AnAction {
    /**
     * 点击事件
     *
     * @param event 事件
    ...
    public void actionPerformed(@NotNull AnActionEvent event) {
        Logger.info("Migration Appraise");
        // 检查当前webView是否已经打开
        ...
        List<VirtualFile> collect = instance.getOpenFiles().stream().filter(virtualFile -> virtualFile.getName()
                .contains(LeftTreeTitleConstant.SOFTWARE_PORTING_ASSESSMENT))
                .collect(Collectors.toList());

        if (collect.isEmpty()) {
            MigrationAppraiseEditor.openPage(LeftTreeTitleConstant.SOFTWARE_PORTING_ASSESSMENT);
        } else {
